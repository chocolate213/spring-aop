 ...
 * LoggingTargetAspect
 *
 * target: target表示目标类中的所有方法匹配切点（不支持通配符）
 *
 * @author zhangjiaxing
 * @version 1.0
 */
@Component
@Aspect
public class LoggingTargetAspect {
    /**
     * 前置通知：在目标方法开始执行前执行
     *
     ...
     */
    @Before("target(cn.jxzhang.demo.spring.service.impl.StudentServiceImpl)")
     ...
     * @param joinPoint join point
     */
    @After("target(cn.jxzhang.demo.spring.service.impl.StudentServiceImpl)")
     ...
     * @param result    返回结果
     */
    @AfterReturning(value = "target(cn.jxzhang.demo.spring.service.impl.StudentServiceImpl)", returning = "result")
     ...
     */
    @AfterThrowing(value = "target(cn.jxzhang.demo.spring.service.impl.StudentServiceImpl)", throwing = "exception")
