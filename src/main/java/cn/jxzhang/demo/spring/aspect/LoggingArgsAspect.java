 ...
 * LoggingArgsAspect
 *
 * 当目标方法传入带有指定类名的入参时，方法匹配（方法入参必须完全匹配，多个参数之间用逗号隔开）
 *
 * @author zhangjiaxing
 * @version 1.0
 */
@Aspect
@Component
public class LoggingArgsAspect {
     ...
     * @param joinPoint join point
     */
    @After("args(cn.jxzhang.demo.spring.domain.Student)")
     ...
     * @param result    返回结果
     */
    @AfterReturning(value = "args(cn.jxzhang.demo.spring.domain.Student)", returning = "result")
     ...
     */
    @AfterThrowing(value = "args(cn.jxzhang.demo.spring.domain.Student)", throwing = "exception")
